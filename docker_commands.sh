...
# The video files must be in an inout/ folder located at the same level as this file.
...
for f in `cat list_of_filenames.txt`
  do
    ...
    docker run --gpus all -v $PWD/inout:/app/inout ubuntu-phalp phalp $f
    ...
    # docker run --gpus all -v $PWD/inout:/app/inout ubuntu-phalp lart $f.phalp.pkl
  done
